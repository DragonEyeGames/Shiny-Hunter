...
class CaptureCard(tk.Frame):
    def __init__(self, parent, back_callback, camera_index=0):
        ...
        self.controller_connecting = False

        self.back_button = tk.Button(
            self, text="Back", font=("Arial", 16), command=back_callback
        )
        self.back_button.place(x=690, y=20, width=100, height=40)

        self.hunting = tk.Label(
            self, bg="black", fg="white", font=("Arial", 20),
            text=f"Hunting {config.pokemon_name} in {config.game_name}"
        )
    ...
    def start_camera(self):
        ...
        # Kick off controller connection in the background so the GUI
        # doesn't freeze while waiting for Bluetooth pairing
        if self.controller is None and not self.controller_connecting:
            self.controller_connecting = True
    ...
    def _connect_controller(self):
        ...
        controller.connect()          # blocks HERE, but on a background thread now
        controller.press_a()
        self.controller = controller  # only touched after connect succeeds
        self.controller_connecting = False
